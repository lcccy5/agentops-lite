...
4. 使用 `curl -N` 请求 `/v1/chat/completions` 并中途终止：`ESTIMATE_FALLBACK` 端点应写入 `USAGE_ESTIMATED` 并最终结算；`QUERYABLE` 端点在已捕获 generation ID 时应进入 `SETTLEMENT_PENDING`，Redis `active` 归零但预占保留，Worker 查询成功或到期后才最终释放预占。
...
用量结算的端点配置、状态口径和管理接口见 [USAGE-SETTLEMENT-IMPLEMENTATION.md](USAGE-SETTLEMENT-IMPLEMENTATION.md)。
