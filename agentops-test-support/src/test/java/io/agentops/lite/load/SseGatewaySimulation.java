...
public final class SseGatewaySimulation extends Simulation {
    ...
    private static final String COMPLETE_REQUEST = request(STEADY_SECONDS > 0 ? "soak-complete" : "load-complete");
    private static final String CANCEL_REQUEST = request(STEADY_SECONDS > 0 ? "soak-slow" : "load-slow");
    ...
    private final ScenarioBuilder complete = scenario("SSE completes")
            ...
            .exec(sse("open-complete").post("/v1/chat/completions")
                    .header("X-AgentOps-Request-Id", "#{requestId}")
                    .header("Idempotency-Key", "#{requestId}").body(StringBody(COMPLETE_REQUEST)).asJson())
            .exec(session -> session.set("stop", false))
            .asLongAs(session -> !session.isFailed() && !session.getBoolean("stop")).on(
    ...
    private final ScenarioBuilder cancel = scenario("SSE client cancels")
            .exec(session -> session.set("requestId", UUID.randomUUID().toString()))
            .exec(sse("open-cancel").post("/v1/chat/completions")
                    .header("X-AgentOps-Request-Id", "#{requestId}")
                    .header("Idempotency-Key", "#{requestId}").body(StringBody(CANCEL_REQUEST)).asJson())
            .pause(Duration.ofMillis(CANCEL_AFTER_MS)).exec(sse("cancel-stream").close());
    ...
    public SseGatewaySimulation() {
        if (STEADY_SECONDS > 0) {
            setUp(
                    complete.injectClosed(
                            rampConcurrentUsers(0).to(COMPLETE_USERS).during(Duration.ofSeconds(Math.max(1, RAMP_SECONDS))),
                            constantConcurrentUsers(COMPLETE_USERS).during(Duration.ofSeconds(STEADY_SECONDS))),
                    cancel.injectClosed(
                            rampConcurrentUsers(0).to(CANCEL_USERS).during(Duration.ofSeconds(Math.max(1, RAMP_SECONDS))),
                            constantConcurrentUsers(CANCEL_USERS).during(Duration.ofSeconds(STEADY_SECONDS)))
            ).protocols(protocol).assertions(global().failedRequests().percent().lte(0.0),
                    global().responseTime().percentile(95.0).lte(MAX_P95_MS));
            return;
        }
        var completeInjection = RAMP_SECONDS > 0
                ? complete.injectOpen(rampUsers(COMPLETE_USERS).during(Duration.ofSeconds(RAMP_SECONDS)))
                : complete.injectOpen(atOnceUsers(COMPLETE_USERS));
        var cancelInjection = RAMP_SECONDS > 0
        ...
        setUp(completeInjection, cancelInjection).protocols(protocol)
