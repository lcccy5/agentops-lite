...
class EvalGatePolicyTest {
  ...
  @Test
  void passesNonRegressingCandidateWithinCostBudget() {
    assertThat(
            policy
                .decide(0, metrics(.8, .8, .7, .9), metrics(.9, .9, .8, .9), 100, 109, 10)
                .passed())
        .isTrue();
  }

  @Test
  void rejectsSafetyAggregateAndCostRegressions() {
    assertThat(
            policy
                .decide(1, metrics(.9, .8, .8, .8), metrics(.8, .8, .8, .8), 100, 121, 10)
                .reasons())
        .hasSize(3);
  }

  @Test
  void rejectsEachHiddenQualityDimensionRegression() {
  ...
  private static QualityMetrics metrics(
      double pass, double tool, double arguments, double evidence) {
