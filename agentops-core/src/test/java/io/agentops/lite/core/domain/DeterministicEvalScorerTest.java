...
class DeterministicEvalScorerTest {
  ...
  @Test
  void passesMatchingObservableBehavior() {
    var expected =
        new EvalCaseDefinition(
            "case",
            "question",
            "fixture",
            List.of("metrics"),
            Map.of("period", mapper.getNodeFactory().textNode("ONE_YEAR")),
            List.of("METRIC"),
            List.of("最近一年"),
            List.of("保证收益"),
            false);
    var actual =
        new AgentEvalObservation(
            "run",
            "最近一年风险较高",
            List.of(
                new ToolObservation(
                    "metrics", mapper.createObjectNode().put("period", "ONE_YEAR"))),
            List.of("METRIC"),
            10,
            5,
            2,
  ...
  @Test
  void explainsEveryFailedConstraint() {
    var expected =
        new EvalCaseDefinition(
            "case",
            "question",
    ...
    var score = scorer.score(expected, actual);
    assertThat(score.failures())
        .containsExactly(
            "missing tool: metrics",
            "missing evidence: METRIC",
            "answer missing: 风险",
            "forbidden claim: 保证收益");
