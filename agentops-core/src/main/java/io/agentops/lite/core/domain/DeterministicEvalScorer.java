...
public final class DeterministicEvalScorer {
  ...
  public Score score(EvalCaseDefinition expected, AgentEvalObservation actual) {
    ...
    for (String tool : safe(expected.expectedTools()))
      if (!tools.contains(tool)) toolFailures.add("missing tool: " + tool);
    for (Map.Entry<String, JsonNode> argument : safeMap(expected.expectedArguments()).entrySet()) {
      boolean matched =
          safe(actual.tools()).stream()
              .anyMatch(
                  tool ->
                      tool.arguments() != null
                          && tool.arguments().findValue(argument.getKey()) != null
                          && tool.arguments()
                              .findValue(argument.getKey())
                              .equals(argument.getValue()));
      if (!matched)
        argumentFailures.add("missing argument: " + argument.getKey() + "=" + argument.getValue());
    }
    for (String evidence : safe(expected.requiredEvidenceTypes()))
      if (!safe(actual.evidenceTypes()).contains(evidence))
        evidenceFailures.add("missing evidence: " + evidence);
    String answer = actual.answer() == null ? "" : actual.answer();
    for (String pattern : safe(expected.requiredAnswerPatterns()))
      if (!answer.contains(pattern)) answerFailures.add("answer missing: " + pattern);
    for (String forbidden : safe(expected.forbiddenClaims()))
      if (answer.contains(forbidden)) answerFailures.add("forbidden claim: " + forbidden);
    ...
    return new Score(
        failures.isEmpty(),
        toolFailures.isEmpty(),
        argumentFailures.isEmpty(),
        evidenceFailures.isEmpty(),
        answerFailures.isEmpty(),
