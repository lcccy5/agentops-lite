...
public final class ProviderUsageLookupWorker {
  ...
  /**
   * Creates the asynchronous query worker using the provider credential kept in worker
   * configuration.
   */
  public ProviderUsageLookupWorker(
      JdbcTemplate jdbc,
    ...
    this.redis = redis;
    this.mapper = mapper;
    this.client = evaluationWebClient;
  ...
  public void resolveProviderUsage() {
    ...
    for (Map<String, Object> task :
        jdbc.queryForList(
            "select"
                + " task_id,reservation_id,provider_generation_id,usage_query_path,provider_base_url,deadline_at,attempts"
                + " from usage_lookup_task where (status='PENDING' and next_attempt_at<=?) or"
                + " (status='PROCESSING' and lease_until<?) order by next_attempt_at limit 50",
            now,
            now)) {
      String taskId = task.get("task_id").toString();
      // A lease recovery may replay a request, so final settlement still checks PROCESSING
      // atomically.
      if (jdbc.update(
              "update usage_lookup_task set"
                  + " status='PROCESSING',lease_owner=?,lease_until=?,updated_at=? where task_id=?"
      ...
      try {
        JsonNode response =
            client
                .get()
                .uri(task.get("provider_base_url") + path(task))
                .headers(headers -> headers.setBearerAuth(properties.providerApiKey()))
                .retrieve()
                .bodyToMono(JsonNode.class)
                .block(Duration.ofSeconds(10));
        JsonNode usage = usageNode(response);
        if (usage == null || usage.isMissingNode() || usage.isNull())
          throw new IllegalStateException("USAGE_NOT_READY");
        settleProviderUsage(
            task,
  ...
  /**
   * Builds a configured lookup path and permits only the captured generation identifier
   * substitution.
   */
  private String path(Map<String, Object> task) {
    return task.get("usage_query_path")
        .toString()
        .replace(
            "{id}",
            java.net.URLEncoder.encode(
                task.get("provider_generation_id").toString(),
                java.nio.charset.StandardCharsets.UTF_8));
  }

  /**
   * Accepts direct and common envelope response shapes while keeping endpoint choice
  ...
  private void settleProviderUsage(Map<String, Object> task, long input, long output) {
    if (input < 0 || output < 0)
      throw new IllegalArgumentException("Provider usage must contain non-negative token counts");
        ...
        transactions.execute(
            status -> {
              Map<String, Object> reservation =
                  jdbc.queryForMap(
                      "select project_id,prompt_version,execution_outcome from usage_reservation"
                          + " where reservation_id=? for update",
                      reservationId);
              int changed =
                  jdbc.update(
                      "update usage_lookup_task set status='COMPLETED',updated_at=? where task_id=?"
              ...
              jdbc.update(
                  "insert into"
                      ...
                      + " values(?,?,?,'USAGE_ACTUAL',?,0,?,?)",
                  ledgerId,
                  reservationId,
                  reservation.get("project_id"),
                  total,
                  reservation.get("prompt_version"),
                  now);
              UsageLedgerEvent event =
                  new UsageLedgerEvent(
                      ledgerId,
                      reservation.get("project_id").toString(),
                      reservationId,
                      "USAGE_ACTUAL",
                      total,
                      BigDecimal.ZERO,
                      (String) reservation.get("prompt_version"),
                      now);
              jdbc.update(
                  "insert into"
              ...
              jdbc.update(
                  "update usage_reservation set"
                      + " actual_tokens=?,input_tokens=?,output_tokens=?,usage_source='PROVIDER_QUERY',status=?,settlement_status='FINAL',updated_at=?"
                      + " where reservation_id=?",
                  total,
                  input,
                  output,
                  reservation.get("execution_outcome"),
                  now,
                  reservationId);
              return true;
            });
  ...
  /**
   * Retries a transient lookup until its deadline, then settles the persisted estimate exactly
   * once.
   */
  private void retryOrEstimate(Map<String, Object> task, String error) {
    Instant now = Instant.now();
    Instant deadline = ((java.sql.Timestamp) task.get("deadline_at")).toInstant();
    int attempts = ((Number) task.get("attempts")).intValue() + 1;
    if (now.isBefore(deadline) && attempts <= properties.usageQueryRetryLimit()) {
      jdbc.update(
          "update usage_lookup_task set"
              + " status='PENDING',attempts=?,next_attempt_at=?,last_error_code=?,updated_at=?"
              + " where task_id=? and status='PROCESSING'",
          attempts,
          now.plusSeconds(Math.min(60, 1L << Math.min(6, attempts))),
          safeError(error),
          now,
          task.get("task_id"));
      return;
    ...
    Long estimate =
        jdbc.queryForObject(
            "select actual_tokens from usage_reservation where reservation_id=?",
            Long.class,
            ...
            status -> {
              Map<String, Object> reservation =
                  jdbc.queryForMap(
                      ...
                      reservationId);
              if (jdbc.update(
                      "update usage_lookup_task set status='EXPIRED',last_error_code=?,updated_at=?"
                          + " where task_id=? and status='PROCESSING'",
                      reason,
                      Instant.now(),
                      task.get("task_id"))
              ...
              jdbc.update(
                  "update usage_reservation set"
                      + " usage_source='ESTIMATED',status=?,settlement_status='FINAL',failure_code=?,updated_at=?"
                      + " where reservation_id=?",
                  reservation.get("execution_outcome"),
                  reason,
                  now,
  ...
  private void finalizeQuota(Map<String, Object> task, long total) {
    String projectId =
        jdbc.queryForObject(
            "select project_id from usage_reservation where reservation_id=?",
            String.class,
            task.get("reservation_id"));
    redis.execute(
        FINALIZE,
        List.of(
            "agentops:quota:" + projectId, "agentops:reservation:" + task.get("reservation_id")),
        Long.toString(total),
        "300000");
  }
  ...
  private String safeError(String value) {
    return value == null ? "LOOKUP_FAILED" : value.substring(0, Math.min(value.length(), 120));
  }

  private static DefaultRedisScript<Long> script() {
    DefaultRedisScript<Long> script = new DefaultRedisScript<>();
    script.setLocation(new ClassPathResource("lua/finalize.lua"));
