...
/**
 * Relays immutable usage facts, applies idempotent projections and reports recoverable
 * discrepancies.
 */
@Component
public final class UsageWorker {
  ...
  public void relayUsageOutbox() {
    List<Map<String, Object>> rows =
        jdbc.queryForList(
            "select event_id,event_key,payload_json from usage_outbox where status='PENDING' and"
                + " next_attempt_at<=? order by created_at limit 100",
            Instant.now());
    for (Map<String, Object> row : rows) {
      ...
      try {
        kafka
            .send(
                "agentops.usage.ledger.v1",
                row.get("event_key").toString(),
                row.get("payload_json").toString())
            .get();
        jdbc.update(
            "update usage_outbox set status='PUBLISHED',published_at=? where event_id=? and"
                + " status='PENDING'",
            Instant.now(),
            eventId);
      } catch (Exception exception) {
        jdbc.update(
            "update usage_outbox set attempts=attempts+1,next_attempt_at=? where event_id=?",
  ...
  /**
   * Applies each ledger ID once even when Kafka redelivers a record. Listener concurrency is capped
   * operationally by the usage topic partition count.
   */
  @KafkaListener(
      topics = "agentops.usage.ledger.v1",
      concurrency = "${agentops.worker.usage-consumer-concurrency:4}")
  public void applyUsageProjection(String payload) throws Exception {
    ...
    transactions.executeWithoutResult(
        status -> {
          try {
            jdbc.update(
                "insert into usage_projection_applied(ledger_id,applied_at) values(?,?)",
                event.ledgerId(),
                Instant.now());
          ...
          int changed =
              jdbc.update(
                  "update usage_projection set"
                      + " total_tokens=total_tokens+?,total_cost=total_cost+?,updated_at=? where"
                      + " project_id=?",
                  event.tokenDelta(),
                  event.costDelta(),
                  Instant.now(),
          ...
          if (changed == 0)
            jdbc.update(
                "insert into usage_projection(project_id,total_tokens,total_cost,updated_at)"
                    + " values(?,?,?,?)",
                event.projectId(),
                event.tokenDelta(),
                event.costDelta(),
  ...
  /**
   * Expires unfinished reservations and reports ledger/projection/consumed divergence. Redis holds
   * are released from MySQL facts: RESERVED refunds without a stub, PENDING still requires one.
   */
  @Scheduled(fixedDelayString = "${agentops.worker.recovery-delay-ms:10000}")
  public void reconcileUsage() {
    Instant now = Instant.now();

    for (Map<String, Object> expired :
        jdbc.queryForList(
"""
...
""",
          ...
          status -> {
            if (jdbc.update(
                    "update usage_reservation set"
                        + " status='CANCELLED',settlement_status='FINAL',execution_outcome='CANCELLED',failure_code='RESERVATION_EXPIRED',updated_at=?"
                        + " where reservation_id=? and status in ('PENDING','RESERVED')",
                    now,
                    reservationId)
                != 1) return;
            jdbc.update(
                ...
                reservationId,
                "expiry-compensate:" + reservationId,
                "COMPENSATE",
                expired.get("reserved_tokens"),
                now,
                now,
                now);
          });
    ...
    for (Map<String, Object> project : jdbc.queryForList("select project_id from agent_project")) {
      ...
      List<Long> projected =
          jdbc.query(
              ...
              (rs, row) -> rs.getLong(1),
              projectId);
      long projection = projected.isEmpty() ? 0 : projected.getFirst();
      long expected = ledger == null ? 0 : ledger;
      if (expected != projection
          && !alreadyOpen(projectId, "LEDGER_PROJECTION", expected, projection))
        discrepancy(
            projectId,
            "LEDGER_PROJECTION",
            expected,
            projection,
            "Replay unpublished usage outbox records or rebuild projection from ledger");
      ...
      if (consumed != expected && !alreadyOpen(projectId, "LEDGER_REDIS", expected, consumed))
        discrepancy(
            projectId,
            "LEDGER_REDIS",
            expected,
            consumed,
            "Inspect expired markers, then reconcile Redis counters from immutable ledger");
    }
  }
  ...
  private void markMissingProviderId(Map<String, Object> expired, Instant now) {
    transactions.executeWithoutResult(
        status -> {
          ...
          if (jdbc.update(
                  "update usage_reservation set"
                      + " status='SETTLEMENT_PENDING',settlement_status='PENDING',execution_outcome='UNKNOWN',failure_code='ID_UNAVAILABLE',updated_at=?"
                      + " where reservation_id=? and status in ('PENDING','RESERVED')",
                  now,
                  reservationId)
              != 1) return;
          jdbc.update(
              ...
              UUID.randomUUID().toString(),
              reservationId,
              "missing-id-release-active:" + reservationId,
              "RELEASE_ACTIVE",
              0,
              now,
              now,
  ...
  private boolean enqueueProviderRecovery(Map<String, Object> expired, Instant now) {
    ...
    if (!"QUERYABLE".equals(String.valueOf(expired.get("settlement_mode")))
        || generationId == null
        || queryPath == null
        || queryPath.toString().isBlank()) return false;
    Boolean queued =
        transactions.execute(
            status -> {
              ...
              if (jdbc.update(
                      "update usage_reservation set"
                          + " status='SETTLEMENT_PENDING',settlement_status='PENDING',execution_outcome='UNKNOWN',settlement_deadline=?,failure_code='RECOVERED_AFTER_EXPIRY',updated_at=?"
                          + " where reservation_id=? and status in ('PENDING','RESERVED')",
                      deadline,
                      now,
                      reservationId)
              ...
              jdbc.update(
                  ...
                  UUID.randomUUID().toString(),
                  reservationId,
                  expired.get("attempt_id"),
                  generationId,
                  queryPath,
                  expired.get("base_url"),
                  now,
                  deadline,
                  now,
                  now);
              jdbc.update(
                  ...
                  UUID.randomUUID().toString(),
                  reservationId,
                  "recovery-release-active:" + reservationId,
                  "RELEASE_ACTIVE",
                  0,
                  now,
                  now,
  ...
  private boolean alreadyOpen(String project, String type, long expected, long actual) {
    Integer count =
        jdbc.queryForObject(
            "select count(*) from usage_reconciliation where project_id=? and discrepancy_type=?"
                + " and expected_value=? and actual_value=? and detected_at>?",
            Integer.class,
            project,
            type,
            expected,
  ...
  private void discrepancy(
      String project, String type, long expected, long actual, String suggestion) {
    jdbc.update(
        "insert into"
            + " usage_reconciliation(reconciliation_id,project_id,discrepancy_type,expected_value,actual_value,suggested_action,detected_at)"
            + " values(?,?,?,?,?,?,?)",
        UUID.randomUUID().toString(),
        project,
        type,
