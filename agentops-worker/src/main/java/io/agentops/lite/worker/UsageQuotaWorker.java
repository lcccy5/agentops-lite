...
public final class UsageQuotaWorker {
  ...
  public UsageQuotaWorker(JdbcTemplate jdbc, StringRedisTemplate redis) {
    this.jdbc = jdbc;
    this.redis = redis;
  }
  ...
  public void applyPendingQuotaTasks() {
    ...
    for (Map<String, Object> task :
        jdbc.queryForList(
            "select"
                + " q.task_id,q.reservation_id,q.operation_id,q.action_type,q.token_value,q.lease_version,r.project_id"
                + " from usage_quota_task q join usage_reservation r on"
                + " r.reservation_id=q.reservation_id where (q.status='PENDING' and"
                + " q.next_attempt_at<=?) or (q.status='PROCESSING' and q.lease_until<=?) order by"
                + " q.created_at limit 100",
            scanTime,
            scanTime)) {
      ...
      if (jdbc.update(
              "update usage_quota_task set"
                  + " status='PROCESSING',lease_owner=?,lease_until=?,lease_version=lease_version+1,updated_at=?"
                  + " where task_id=? and lease_version=? and ((status='PENDING' and"
                  + " next_attempt_at<=?) or (status='PROCESSING' and lease_until<=?))",
              leaseOwner,
      ...
      try {
        ...
        int completed =
            jdbc.update(
                "update usage_quota_task set"
                    + " status='APPLIED',lease_owner=NULL,lease_until=NULL,updated_at=? where"
                    + " task_id=? and status='PROCESSING' and lease_owner=? and lease_version=?",
                completedAt,
                task.get("task_id"),
                leaseOwner,
                leaseVersion);
        if (completed == 1)
          jdbc.update(
              "update usage_reservation set quota_sync_status='APPLIED',updated_at=? where"
                  + " reservation_id=?",
              completedAt,
        ...
        int retried =
            jdbc.update(
                "update usage_quota_task set"
                    + " status='PENDING',attempts=attempts+1,next_attempt_at=?,last_error_code=?,lease_owner=NULL,lease_until=NULL,updated_at=?"
                    + " where task_id=? and status='PROCESSING' and lease_owner=? and"
                    + " lease_version=?",
        ...
        if (retried == 1)
          jdbc.update(
              "update usage_reservation set quota_sync_status='FAILED',updated_at=? where"
                  + " reservation_id=?",
              failedAt,
              task.get("reservation_id"));
      }
  ...
  private void apply(Map<String, Object> task) {
    ...
    Long result =
        switch (task.get("action_type").toString()) {
          case "FINALIZE" ->
              redis.execute(FINALIZE, List.of(project, reservation), value, "604800000");
          case "RELEASE_ACTIVE" ->
              redis.execute(RELEASE_ACTIVE, List.of(project, reservation), "604800000");
          case "COMPENSATE" ->
              redis.execute(COMPENSATE, List.of(project, reservation), "604800000", value, "1");
          case "ADJUST" ->
              redis.execute(
                  ADJUST,
                  List.of(project, "agentops:quota-operation:" + task.get("operation_id")),
                  value,
                  "604800000");
          default ->
              throw new IllegalStateException(
                  "Unknown quota task action " + task.get("action_type"));
        };
    if (result == null || result == 0L)
      throw new IllegalStateException("Redis quota marker is unavailable");
  }

  /**
   * Truncates retry diagnostics to the schema limit without persisting credentials or response
   * bodies.
   */
