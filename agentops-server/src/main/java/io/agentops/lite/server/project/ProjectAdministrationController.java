...
public final class ProjectAdministrationController {
  ...
  public ProjectAdministrationController(ProjectAdministrationService projects) {
  ...
  public Map<String, Object> queryProjectOverview(@PathVariable String projectId) {
    return projects.queryProjectOverview(projectId);
  }

  /** Updates future admission limits for one isolated Agent product. */
  @PostMapping("/updateProjectQuota/{projectId}")
  public Map<String, Object> updateProjectQuota(
      @PathVariable String projectId,
      @RequestBody ProjectAdministrationService.UpdateQuotaRequest request) {
    return projects.updateProjectQuota(projectId, request);
  }

  /** Creates a new API key and returns its plaintext value exactly once. */
  @PostMapping("/createProjectApiKey/{projectId}")
  public Map<String, Object> createProjectApiKey(
      @PathVariable String projectId,
      @RequestBody ProjectAdministrationService.CreateApiKeyRequest request) {
    return projects.createProjectApiKey(projectId, request);
  }

  /** Lists non-secret API key metadata for one Agent product. */
  @GetMapping("/queryProjectApiKeys/{projectId}")
  public List<Map<String, Object>> queryProjectApiKeys(@PathVariable String projectId) {
    return projects.queryProjectApiKeys(projectId);
  }
