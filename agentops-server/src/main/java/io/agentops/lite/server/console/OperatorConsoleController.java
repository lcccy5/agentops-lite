...
public final class OperatorConsoleController {
  ...
  public OperatorConsoleController(
      UsageService usage, ControlPlaneService controlPlane, JdbcTemplate jdbc) {
    this.usage = usage;
  ...
  public String queryRecentRuns(ServerWebExchange exchange) {
    ...
    for (Map<String, Object> run : usage.queryRecentRuns(project(exchange), 100)) {
      ...
      rows.append("<tr><td><a href='/console/requests/")
          .append(escape(id))
          .append("'>")
          .append(shortId(id))
          .append("</a></td><td>")
          .append(escape(value(run, "prompt_version")))
          .append("</td><td>")
          .append(escape(value(run, "model_call_count")))
          .append("</td><td>")
          .append(escape(value(run, "actual_tokens")))
          .append("</td><td><span class='status'>")
          .append(escape(value(run, "settlement_status")))
          .append("</span></td><td>")
          .append(escape(value(run, "started_at")))
          .append("</td></tr>");
    }
    return page(
        "调用记录",
        "一次 FundPilot 运行可关联多次真实模型调用；账本仍按调用独立结算。",
  ...
  public String queryRunDetails(@PathVariable String correlationId, ServerWebExchange exchange) {
    ...
    List<Map<String, Object>> calls = (List<Map<String, Object>>) run.get("calls");
    StringBuilder rows = new StringBuilder();
    for (Map<String, Object> call : calls) {
      rows.append("<tr><td>")
          .append(shortId(value(call, "request_id")))
          .append("</td><td>")
          .append(escape(value(call, "status")))
          .append("</td><td>")
          .append(escape(value(call, "settlement_status")))
          .append("</td><td>")
          .append(escape(value(call, "reserved_tokens")))
          .append("</td><td>")
          .append(escape(value(call, "actual_tokens")))
          .append("</td><td>")
          .append(escape(value(call, "usage_source")))
          .append("</td><td>")
          .append(escape(value(call, "quota_sync_status")))
          .append("</td><td>")
          .append(escape(value(call, "failure_code")))
          .append("</td></tr>");
    }
    String cards =
        "<div class='cards'><div><b>模型调用</b><strong>"
            + run.get("modelCallCount")
            + "</strong></div><div><b>预占 Token</b><strong>"
            + run.get("reservedTokens")
            + "</strong></div><div><b>实际 Token</b><strong>"
            + run.get("actualTokens")
            + "</strong></div><div><b>全部结算</b><strong>"
            + run.get("settled")
            + "</strong></div></div>";
    return page(
        "运行详情",
        "Correlation ID · " + escape(correlationId),
        cards
            + table(
                List.of("Request ID", "执行状态", "结算状态", "预占", "实际", "用量来源", "Redis 同步", "失败码"),
                rows));
  }
...
""",
    ...
    for (Map<String, Object> job : jobs) {
      rows.append("<tr><td>")
          .append(shortId(value(job, "job_id")))
          .append("</td><td>")
          .append(escape(value(job, "prompt_key")))
          .append("</td><td>")
          .append(escape(value(job, "stable_version")))
          .append(" → ")
          .append(escape(value(job, "candidate_version")))
          .append("</td><td>")
          .append(escape(value(job, "result_count")))
          .append(" / ")
          .append(escape(value(job, "passed_count")))
          .append("</td><td><span class='status'>")
          .append(escape(gateState(job)))
          .append("</span></td><td class='reason'>")
          .append(escape(value(job, "reasons_json")))
          .append("</td></tr>");
    }
    return page(
        "评测门禁",
        "真实 Agent 的确定性评测结果；硬安全失败或任一维度退化都会阻止发布。",
...
""",
    ...
    for (Map<String, Object> release : releases) {
      ...
      String action =
          active
              ? "<form method='post' action='/console/releases/rollbackRelease/"
                  + escape(value(release, "release_id"))
                  + "'><button type='submit'>回滚到稳定版</button></form>"
              : "—";
      rows.append("<tr><td>")
          .append(escape(value(release, "prompt_key")))
          .append("</td><td>")
          .append(escape(value(release, "environment_name")))
          .append("</td><td>")
          .append(escape(value(release, "stable_version")))
          .append("</td><td>")
          .append(escape(value(release, "candidate_version")))
          .append("</td><td>")
          .append(escape(value(release, "canary_percent")))
          .append("%</td><td><span class='status'>")
          .append(escape(value(release, "status")))
          .append("</span></td><td>")
          .append(action)
          .append("</td></tr>");
    }
    return page(
        "发布治理",
        "发布只能引用已通过的 Gate；5% 灰度稳定分桶，回滚立即恢复稳定版本。",
  ...
  public ResponseEntity<Void> rollbackRelease(
    ...
    return ResponseEntity.status(303)
        .header(HttpHeaders.LOCATION, "/console/releases?projectId=" + projectId)
        .build();
  }
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            nav a { color: #cbd5e1; text-decoration: none; margin-right: 22px; }
            main { max-width: 1280px; margin: 34px auto; padding: 0 24px; }
            ...
            .panel {
              ...
              overflow: auto;
              margin-top: 24px;
              box-shadow: 0 8px 30px #1e293b0d;
            }
            ...
            th { font-size: 12px; color: #9eb1ca; background: #12273f; }
            .reason { max-width: 420px; white-space: normal; }
            .status { background: #123d3c; color: #5ce0c0; border-radius: 999px; padding: 5px 9px; font-size: 12px; }
            a { color: #2463eb; }
            .cards { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-top: 24px; }
            .cards div { background: #0f2034; border: 1px solid #29445f; border-radius: 14px; padding: 18px; }
            .cards b { display: block; color: #718096; font-size: 13px; }
            .cards strong { display: block; font-size: 26px; margin-top: 10px; }
  ...
  private String table(List<String> headers, StringBuilder rows) {
    ...
    String content =
        rows.isEmpty()
            ? "<tr><td colspan='" + headers.size() + "'>暂无数据</td></tr>"
            : rows.toString();
  ...
  private String gateState(Map<String, Object> job) {
    ...
    return passed == null
        ? value(job, "status")
        : Boolean.TRUE.equals(passed) ? "PASSED" : "REJECTED";
  }
