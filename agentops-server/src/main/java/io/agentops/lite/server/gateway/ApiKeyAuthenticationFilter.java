...
public final class ApiKeyAuthenticationFilter implements WebFilter {
    ...
    /**
     * Resolves an administrator-selected project before the request reaches a controller.
     * The database check runs on the configured blocking scheduler because JdbcTemplate must never occupy a Netty event loop.
     */
    private Mono<Void> attachManagementProject(ServerWebExchange exchange, WebFilterChain chain) {
        return Mono.fromCallable(() -> managementProject(exchange)).subscribeOn(blockingScheduler).flatMap(projectId -> {
            exchange.getAttributes().put(PROJECT_ATTRIBUTE, projectId);
            return chain.filter(exchange);
        });
    }
    ...
    /**
     * Accepts an explicit project header for API tools or a projectId query value for browser navigation.
     * It retains the seeded project as a compatibility default but rejects any non-existent selection.
     */
    private String managementProject(ServerWebExchange exchange) {
        ...
        if (selected == null || selected.isBlank()) selected = exchange.getRequest().getQueryParams().getFirst("projectId");
        if (selected == null || selected.isBlank()) selected = "project-fund-agent";
        Integer exists = jdbc.queryForObject("select count(*) from agent_project where project_id=?", Integer.class, selected);
        if (exists == null || exists != 1) throw new GatewayException("PROJECT_NOT_FOUND", HttpStatus.NOT_FOUND, "Selected project does not exist");
        return selected;
    }
