 ...
 * virtual-thread executor, while controllers that already return a reactive Publisher keep their
 * normal non-blocking execution path.
 */
...
public final class BlockingWebFluxConfiguration implements WebFluxConfigurer {
  ...
  /**
   * Delegates only synchronous handler methods to the blocking executor. Reactive handlers such as
   * the streaming chat gateway remain on their native Reactor path.
   *
  ...
  public void configureBlockingExecution(BlockingExecutionConfigurer configurer) {
    ...
    configurer.setControllerMethodPredicate(
        handler -> !Publisher.class.isAssignableFrom(handler.getMethod().getReturnType()));
  }
}
