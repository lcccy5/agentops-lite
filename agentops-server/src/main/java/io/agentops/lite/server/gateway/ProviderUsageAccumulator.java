...
/**
 * Parses an SSE stream across arbitrary network chunks and retains only settlement evidence.
 * Raw diagnostic capture may be capped, but this accumulator must receive every chunk so a
 * terminal usage event after the diagnostic cap is still authoritative.
 */
final class ProviderUsageAccumulator {
    ...
    ProviderUsageAccumulator(ObjectMapper mapper) { this.mapper = mapper; }

    /**
     * Accepts one arbitrary byte chunk. Parsing waits for a blank-line event boundary so JSON
     * split by TCP framing cannot be treated as a malformed or missing usage event.
     */
    ...
    /** Returns the provider's final usage when present, otherwise a versioned visible-content estimate. */
    ConfirmedUsage usage(JsonNode request) {
        if (latestUsage != null && latestUsage.hasNonNull("prompt_tokens") && latestUsage.hasNonNull("completion_tokens")) {
            return new ConfirmedUsage(latestUsage.path("prompt_tokens").asLong(), latestUsage.path("completion_tokens").asLong(), false);
        }
    ...
    private static boolean validUsage(JsonNode usage) {
        return usage != null && usage.path("prompt_tokens").canConvertToLong() && usage.path("completion_tokens").canConvertToLong()
                && usage.path("prompt_tokens").longValue() >= 0 && usage.path("completion_tokens").longValue() >= 0;
    }

    /** Returns the generation ID only after it has been parsed from a complete event. */
    String generationId() { return generationId; }
    ...
    private static int boundaryWidth(byte[] value, int boundary) {
        return boundary + 3 < value.length && value[boundary] == '\r' && value[boundary + 1] == '\n'
                && value[boundary + 2] == '\r' && value[boundary + 3] == '\n' ? 4 : 2;
    }
    ...
    private void parseEvent(String event) {
        for (String line : event.split("\\R")) {
            ...
            try {
                ...
                String id = node.path("id").asText(""); if (!id.isBlank()) generationId = id;
                JsonNode usage = node.path("usage"); if (!usage.isMissingNode() && !usage.isNull()) latestUsage = usage;
            ...
            } catch (Exception ignored) { /* Invalid provider events remain diagnostic-only. */ }
        }
    }
}
